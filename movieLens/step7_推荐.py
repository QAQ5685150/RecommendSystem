...
def creat_user_cluster(cluster_num):
    file = "D:\\development\\py_project\\hello_world\\movieLens\\实验数据\\user_id_Cluster_T1=100,T2=100.xls"
...
def creat_user_movie_dic(cluster):
    dic={}
    file = "D:\\development\\py_project\\hello_world\\movieLens\\实验数据\\实验数据_test_clear.csv"
...
def Recommend(user, dicc, W2, K, rule):
    rvi = 1  # 这里都是1
    rank = dict()
    related_user = []
    ...
    for v, wuv in sorted(related_user, key=itemgetter(1), reverse=True)[0:K]:
        ...
        for i in dicc[v]:   #找到相关用户v的观影列表，遍历每一部电影
            ...
            if i in rule:
                rank[i] += rule.get(i)  # 如果有关联规则存在，推荐值加上该规则支持度
                rule[i] = 0   # 添加过的关联规则数值 删除掉
...
if __name__ == '__main__':
    ...
    data=pd.read_excel(file)
    #print(data)
    for i in range(2,3,1):#用户id  给前50个用户推荐 形成推荐列表
        dic = {}
        cluster=[]
        rule = {}
        cluster_num=find_Cluster(i)#找到用户在哪个簇
        # print(cluster_num)
        apriori_by_clusterNum = pd.read_csv("D:\\development\\recommandSystem\\recommandSystem_experience\\movieLens\\实验数据\\apriori_cluster{}_test.csv".format(int(cluster_num)))
        rule_list = np.array(apriori_by_clusterNum).tolist()
        for k,support,item in rule_list:
            rule.setdefault(item,support)
        # print(rule)
        ...
        #print(W3)
        Last_Rank = Recommend(i, dic, W3, 30,rule)  # 得到推荐列表
        # print(Last_Rank)
        d_order = sorted(Last_Rank.items(), key=lambda x: x[1],
                         reverse=True)[0:30]  # 按字典集合中，每一个元组的第二个元素排列。reverse=True为从大到小排序
        print(d_order)  # 排序后的推荐列表

        outfile="D:\\development\\recommandSystem\\recommandSystem_experience\\movieLens\\实验数据\\T1=100,T2=100_recommend_处理前\\user_{}_k邻=30_recommend.xls".format(i)
